...
const components = {
  ...
  types: {
    socialEmbed: ({ value }: any) => {
      if (!value?.platform || !value?.url) return null;
  ...
  block: {
    normal: ({ children }: any) => (
      <div className="mb-7 text-[18px] leading-[1.78] text-black/80">
        {children}
      </div>
    ),
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="my-16 border-y border-black py-8">
        <div className="mx-auto max-w-2xl text-center text-3xl font-black uppercase leading-[1.05] tracking-[-0.04em] text-black md:text-4xl">
          “{children}”
        </div>
      </blockquote>
    ),
